"""Adapter for OpenAI Codex CLI (openai/codex, Apache-2.0).

Integration surface, per docs/architecture/00-foundational-architecture.md
section 4.4: `codex exec --json` for one-shot task execution,
`codex exec resume <SESSION_ID>` for continuity, `-C`/`--sandbox` for
workspace/sandbox scoping. Flags verified live against `codex exec --help`
and `codex exec resume --help` on the installed 0.122.0 build. Codex is
Apache-2.0 and ships an `app-server-protocol` crate, but per the architecture
decision the stable surface stays the CLI, not the Rust internals — those
move fast (daily alpha releases) and aren't a committed API.
"""
...
class _SessionState:
    def __init__(self, workspace: Workspace, sandbox: str) -> None:
...
class CodexRuntimeAdapter(RuntimeAdapter):
    ...
    async def install(self, spec: RuntimeSpec) -> None:
        ...
        if spec.source.version not in installed:
            # Not fatal by itself — Codex ships alpha builds frequently — but
            # surfaced so the scheduler can flag an upgrade-review task per
            # the Detect -> Review -> Test -> Approve -> Deploy workflow.
            raise RuntimeAdapterError(
                ErrorFamily.VALIDATION,
                f"installed Codex '{installed}' does not match pinned "
                f"version '{spec.source.version}' — see the adapter's runtime.yaml",
            )

    async def configure(self, config: RuntimeConfig) -> None:
        sandbox = config.extra.get("sandbox", "workspace-write")
        if sandbox not in _SANDBOX_MODES:
            raise RuntimeAdapterError(ErrorFamily.VALIDATION, f"invalid sandbox mode {sandbox!r}")
    ...
    async def start(self, session: SessionRequest) -> SessionHandle:
        state = _SessionState(session.workspace, self._pending_sandbox)
    ...
    async def send_task(self, handle: SessionHandle, task: Task) -> TaskAck:
        state = self._require(handle)
        if state.native_ref:
            args = [_CODEX_BIN, "exec", "resume", state.native_ref, task.instructions, "--json"]
        else:
            args = [
                _CODEX_BIN,
                "exec",
    ...
    async def _pump_events(self, session_id: str, state: _SessionState) -> None:
        ...
        async for raw_line in state.process.stdout:
            ...
            try:
                ...
                continue
            # Codex's --json event schema (thread/turn/item/error) is not yet
            # exercised end-to-end here; capture whatever id field is present
            # so resume works, and pass the full payload through untouched.
            native_id = payload.get("thread_id") or payload.get("session_id") or payload.get("id")
            if native_id:
                state.native_ref = native_id
            await state.queue.put(
                RuntimeEvent(
                    ...
                    error_family=(
                        ErrorFamily.UNKNOWN if payload.get("type") in ("error", "turn.failed") else None
                    ),
                )
            )
